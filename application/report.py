...
def read_campbell_scientific(filename, index_col='TIMESTAMP', encoding=None):
    ...
    try:
        df = pd.read_csv(filename, skiprows=1, encoding=encoding, index_col=index_col, parse_dates=True,
                         dtype='unicode', error_bad_lines=False)  # , low_memory=False)
        df = df[2:]
...
def write_dashboard(column_names, row_names, content, title='Pecos Dashboard',
    ...
    html_string = _html_template_dashboard(column_names, row_names, content, title, footnote, logo, im_width,
                                           datatables, encode)
