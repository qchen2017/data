...
def plot_timeseries(data, tfilter=None, test_results_group=None, xaxis_min=None, 
    ...
    tfilter : pandas Series (optional)
        Boolean array used to include time filter in the plot, default = None
    ...
    try:
        ...
        if test_results_group is not None:
            ...
            for error_flag in key2.unique():
                ...
                if len(test_results_group2.index.array) > 4:
                    warning_label = '\n'.join(textwrap.wrap('Warning ' +
                                                            str(test_results_group2.index.array[0:4]).strip('[]') + '...', 30))
                else:
                    warning_label = '\n'.join(textwrap.wrap('Warning ' +
                                                            str(test_results_group2.index.array).strip('[]'), 30))
                ...
                #    color = 'b'
                try:
                    ax.scatter(data2.index, data2.array, marker='+',  # c=color,
                               linewidths=1, label=error_label)   
                except:
                    ax.scatter(data2.index[0], data2.array[0], marker='+',  # c=color,
                               linewidths=1, label=error_label) 
        ...
        ymin_plt, ymax_plt = plt.ylim()
        if tfilter is not None:
            ymin_plt = np.nanmin(data[tfilter].array)
            ymax_plt = np.nanmax(data[tfilter].array)
